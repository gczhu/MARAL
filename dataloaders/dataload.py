...
class MLMDataset(Dataset):
	...
	def __init__(self, args, json_path, tokenizer: BertWordPieceTokenizer, max_length: int = 128, mask_rate: float = 0.25):
		self.args = args
	...
	def __getitem__(self, item):
		if 'bert-base-multilingual-cased' in self.args.bert_config_dir:
			cls_tok = "[CLS]"
			sep_tok = "[SEP]"
			mask_tok = '[MASK]'
		else:
...
class BERTNERDataset(Dataset):
	...
	def __init__(self, args, json_path, tokenizer: BertWordPieceTokenizer, max_length: int = 128, pad_to_maxlen=False, return_guid=False):
		...
		self.class_counts = torch.zeros(self.args.n_class, dtype=torch.int64)
		for data in self.all_data:		# count span num of different classes
			context = data["context"].strip()
			if '\u200b' in context:
				context = context.replace('\u200b', '')
			elif '\ufeff' in context:
				...
				context = context.replace('\u200f', '')
			span_num = len(enumerate_spans(context.split(), offset=0, max_span_width=self.args.max_spanLen))

			# count entity span num
			span_idxLab = data["span_posLabel"]
			for seidx, label in span_idxLab.items():
				idx = label2idx[label]
				self.class_counts[idx] += 1
			# count non-entity span num
			idx = label2idx['O']
			self.class_counts[idx] += span_num - len(span_idxLab)
		self.class_weights = 1 / self.class_counts.log()
		self.class_weights = self.class_weights / torch.sum(self.class_weights, dim=-1, keepdim=True)
		self.class_weights = self.class_weights * len(self.class_weights)
	...
	def __getitem__(self, item):
		...
		for span_idx in all_span_idxs:
			label_idx = label2idx['O']
			weight = self.class_weights[label_idx]
			if span_idx in pos_span_idxs:
				idx = pos_span_idxs.index(span_idx)
	...
	def convert2tokenIdx(self, words, tokens, type_ids, offsets, span_idxs, span_idxLab):
		...
		for start, end in zip(sidxs, eidxs):
			try:
				...
				print(f'problematic span: start={words[span_idxs[n_span_keep][0]]}, end={words[span_idxs[n_span_keep][1]]}')
				print('n_span_keep', n_span_keep)
				print('words', words)
				print('tokens', tokens)
...
class BERTNERDataset_U(Dataset):
	...
	def __init__(self, args, json_path, tokenizer: BertWordPieceTokenizer, max_length: int = 128, pad_to_maxlen=False, mask_rate: float=0.25):
...
class DataReader:
    ...
    def read_data(self, m):
        original_length = len(self.all_data)
        expanded_length = int(original_length * m)
        
        additional_length = expanded_length - original_length
        
        if m <= 1:
            return self.all_data[:expanded_length]
        
        copy_ratio = additional_length / original_length
        
        full_copies = int(copy_ratio)
        partial_copy_ratio = copy_ratio - full_copies
        
        for _ in range(full_copies):
            self.all_data.extend(self.all_data[:])
        
        partial_copy_length = int(original_length * partial_copy_ratio)
        self.all_data.extend(self.all_data[:partial_copy_length])
