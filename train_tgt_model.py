...
from distribution_modeling import DistModeling
...
class BertNerTagger(pl.LightningModule):
    ...
    def __init__(self, args: argparse.Namespace):
        ...
        ul_cls_n_list = torch.zeros(self.args.n_class, dtype=torch.long).cuda()
        for sample in init_confidence_list:
            hard_pseudo_labels = torch.tensor(sample).argmax(dim=1)
            ul_cls_n_list += torch.bincount(hard_pseudo_labels, minlength=self.args.n_class).cuda()
        print("pseudo label class counts: ", ul_cls_n_list)
        
        self.ul_cls_n_list = sum([torch.tensor(sample).sum(dim=0) for sample in init_confidence_list]).cuda()
        self.ul_cls_p_list = self.ul_cls_n_list / self.ul_cls_n_list.sum()
        self.ul_m_list = (args.ul_tau * torch.log(self.ul_cls_p_list)).cuda()
        ...
        self.sel_stats = {
            'pre_score': torch.zeros_like(init_confidence).cuda(),
            'true_labels': torch.ones((init_confidence.shape[0],init_confidence.shape[1]),dtype=torch.long).cuda()*(-1),
            'is_rel': torch.ones(init_confidence.shape[0],init_confidence.shape[1]).cuda(),
            'best_rel': torch.ones(init_confidence.shape[0],init_confidence.shape[1]).cuda(),
            'is_val': torch.ones(init_confidence.shape[0],init_confidence.shape[1]).cuda(),
            'is_val_l': torch.ones(len(self.train_dataloader().dataset),init_confidence.shape[1]).cuda(),
            'true_labels_l': torch.ones((len(self.train_dataloader().dataset),init_confidence.shape[1]),dtype=torch.long).cuda()*(-1),
            'ul_q': torch.ones(init_confidence.shape[0],init_confidence.shape[1],self.args.q_dim).cuda(),
            'ul_contrast_logits': torch.ones(init_confidence.shape[0],init_confidence.shape[1],self.args.n_class).cuda()
        }

        self.train_class_counts = self.train_class_counts.cuda()
        self.cls_p_list = self.train_class_counts / self.train_class_counts.sum()
        self.m_list = (args.tau * torch.log(self.cls_p_list))

        self.dist_model = DistModeling(contrast_dim=self.args.q_dim, temperature=self.args.temp, num_classes=self.args.n_class).cuda()
    ...
    def get_parser():
        ...
        parser.add_argument("--bert_max_length", type=int, default=64, help="max sequence length")
        ...
        parser.add_argument("--ckpt_path", type=str, help='the file path of the best model checkpoint')

        parser.add_argument("--q_dim", type=int, default=64, help='embedding dim of the compressed span rep')
        ...
        parser.add_argument('--mask_rate', type=float, default=0.25, help='mask rate for mlm task')
        ...
        parser.add_argument('--tau', default=1.0, type=float, help='scalar temperature of logit adjustment for labeled data')
        parser.add_argument('--ul_tau', default=0.1, type=float, help='scalar temperature of logit adjustment for unlabeled data')
        parser.add_argument('--save_best_model', default=True, type=bool, help='whether to save the best model')
    ...
    def compute_loss(self, loadall, all_span_logits, span_label_ltoken, real_span_mask_ltoken, mode=None):
        '''
    ...
    def on_train_epoch_start(self):
        ...
        if self.current_epoch > self.args.update_soft_start:
            self.clean_sample_selection()
        self.loss_fn_ul.set_conf_ema_m(self.current_epoch, self.args)
        
        if hasattr(self.dist_model, "_hook_before_epoch"):
            self.dist_model._hook_before_epoch(self.current_epoch, self.args.max_epochs)


    def clean_sample_selection(self):
        valid_token_idx = torch.where(self.sel_stats['is_val'])     # sel_stats['is_val']->(sample_num, num_span_upperb) valid_token_idx selects the index that is equal to 1, i.e., selects the real span and filters out the padding span
        
        valid_pseudo_label_soft = self.loss_fn_ul.confidence[valid_token_idx]   # (valid_token_num, n_class)
        valid_pseudo_label_hard = valid_pseudo_label_soft.max(dim=-1)[1]    # (valid_token_num)
        
        conf_pseudo_label = self.sel_stats['ul_contrast_logits'][valid_token_idx]   # (valid_token_num, n_class)
        conf_pseudo_label = conf_pseudo_label[torch.arange(conf_pseudo_label.size(0)), valid_pseudo_label_hard]     # (valid_token_num)
        
        chosen_idx_clean_label = []
        chosen_idx_noisy_label = []

        for j in range(valid_pseudo_label_soft.shape[-1]):
            idx_j = torch.where(valid_pseudo_label_hard==j)[0]
            conf_pseudo_label_j = conf_pseudo_label[idx_j].cpu()
            conf_pseudo_label_j_nonorm = ((conf_pseudo_label_j-conf_pseudo_label_j.min())/(conf_pseudo_label_j.max()-conf_pseudo_label_j.min()))
            conf_pseudo_label_j_norm = conf_pseudo_label_j_nonorm.reshape(-1,1)
            gmm = GaussianMixture(n_components=2,max_iter=10,reg_covar=5e-4,tol=1e-2)
            
            gmm.fit(conf_pseudo_label_j_norm)
            prob = gmm.predict_proba(conf_pseudo_label_j_norm)
            prob = prob[:,gmm.means_.argmax()]
            chosen_idx_gmm_j = np.where(prob > 0.5)[0]
            chosen_idx_clean_label.append(idx_j[chosen_idx_gmm_j])
            chosen_idx_gmm_j_noisy = np.where(prob <= 0.5)[0]
            chosen_idx_noisy_label.append(idx_j[chosen_idx_gmm_j_noisy])
        
        chosen_idx_clean_label = torch.cat(chosen_idx_clean_label,dim=0)
        chosen_idx_noisy_label = torch.cat(chosen_idx_noisy_label,dim=0)

        self.sel_stats['is_rel'] = torch.zeros_like(self.sel_stats['is_val'])
        rel_pos_x = valid_token_idx[0][(chosen_idx_clean_label.cpu().numpy())]
        rel_pos_y = valid_token_idx[1][(chosen_idx_clean_label.cpu().numpy())]
        self.sel_stats['is_rel'][rel_pos_x,rel_pos_y] = 1   # select clean label samples
    ...
    def training_step(self, batch, batch_idx):
        ...
        tokens, token_type_ids, all_span_idxs_ltoken, morph_idxs, span_label_ltoken, all_span_lens, all_span_weights, real_span_mask_ltoken, words, all_span_word, all_span_idxs, label_guid = batch
        
        real_val_pad = torch.zeros([real_span_mask_ltoken.shape[0], self.num_span_upperb-real_span_mask_ltoken.shape[1]]).to(self.device)
        real_val_pad = torch.cat([real_span_mask_ltoken,real_val_pad],dim=1)
        self.sel_stats['is_val_l'][label_guid] = real_val_pad
        self.sel_stats['true_labels_l'][label_guid,:real_span_mask_ltoken.shape[1]] = span_label_ltoken.detach().clone()
        ...
        all_span_logits = self.dist_model(q.view(-1, self.args.q_dim), span_label_ltoken.view(-1), real_span_mask_ltoken.bool().view(-1))
        all_span_logits = all_span_logits.view(span_label_ltoken.shape[0], span_label_ltoken.shape[1], -1)
        all_span_logits = all_span_logits.to(device=span_label_ltoken.device)
        ...
        all_span_logits = all_span_logits + self.m_list
        ner_loss = self.compute_loss(loadall, all_span_logits, span_label_ltoken, real_span_mask_ltoken, mode='train')

        src_real_span_q_reps = q.detach()[real_span_mask_ltoken.bool()]
        src_real_span_labels = span_label_ltoken.detach()[real_span_mask_ltoken.bool()]
        ...
        is_rel_batch = self.sel_stats['is_rel'][unlabel_guid,:ul_real_span_mask_ltoken.shape[1]]    # clean lable samples
        
        val_pad = torch.zeros([ul_real_span_mask_ltoken.shape[0], self.num_span_upperb-ul_real_span_mask_ltoken.shape[1]]).to(self.device)
        val_pad = torch.cat([ul_real_span_mask_ltoken, val_pad],dim=1)
        self.sel_stats['is_val'][unlabel_guid] = val_pad
        ...
        tgt_pseudo_labels = torch.max(self.loss_fn_ul.confidence[unlabel_guid, :ul_all_span_logits.shape[1]], dim=-1)[1]
        
        selected_ul_real_span_mask_ltoken = ul_real_span_mask_ltoken*is_rel_batch
        
        ul_all_span_logits = self.dist_model(ul_q.view(-1, self.args.q_dim), tgt_pseudo_labels.view(-1), ul_real_span_mask_ltoken.bool().view(-1))
        ul_all_span_logits = ul_all_span_logits.view(tgt_pseudo_labels.shape[0], tgt_pseudo_labels.shape[1], -1)
        ul_all_span_logits = ul_all_span_logits.to(device=tgt_pseudo_labels.device)
        self.sel_stats['ul_contrast_logits'][unlabel_guid,:ul_real_span_mask_ltoken.shape[1]] = ul_all_span_logits.detach().clone()
        
        ul_predict_probs = self.classifier(ul_all_span_logits + self.ul_m_list)
        # ul_predict_probs = self.classifier(ul_all_span_logits)
        
        self.sel_stats['ul_q'][unlabel_guid,:ul_real_span_mask_ltoken.shape[1]] = ul_q.detach().clone()
        self.sel_stats['true_labels'][unlabel_guid,:ul_real_span_mask_ltoken.shape[1]] = ul_span_label_ltoken.detach().clone()
        
        output['unlabel_pseudo_label'] = tgt_pseudo_labels[ul_real_span_mask_ltoken.bool()]

        tgt_real_span_q_reps = ul_q.detach()[ul_real_span_mask_ltoken.bool()]
        tgt_real_span_pseudo_labels = tgt_pseudo_labels.detach()[ul_real_span_mask_ltoken.bool()]
        tgt_real_span_true_labels = ul_span_label_ltoken.detach()[ul_real_span_mask_ltoken.bool()]
        
        downsample_mask = torch.rand(tgt_real_span_pseudo_labels.shape).to(self.device) < sample_rate
        downsample_mask = torch.logical_or(downsample_mask, tgt_real_span_pseudo_labels != 0)
        tgt_real_span_q_reps = tgt_real_span_q_reps[downsample_mask]
        tgt_real_span_pseudo_labels = tgt_real_span_pseudo_labels[downsample_mask]
        tgt_real_span_true_labels = tgt_real_span_true_labels[downsample_mask]

        output['tgt_q_reps'], output['tgt_q_labels'], output['tgt_true_labels'] = tgt_real_span_q_reps, tgt_real_span_pseudo_labels, tgt_real_span_true_labels
        
        logits_prot = ul_all_span_logits.clone().detach()
        probs_prot = F.softmax(logits_prot, dim=-1)
        output['logits_prot'] = logits_prot[ul_real_span_mask_ltoken.bool()]
        output['probs_prot'] = probs_prot[ul_real_span_mask_ltoken.bool()]
        
        
        # Update soft pseudo labels (based on global guidance)
        if self.current_epoch > self.args.update_soft_start:
        ...
        # Update soft pseudo labels (based on local guidance)
        if self.current_epoch > self.args.update_soft_start:
            ...
            classwise_neigh_margin = self.neigh_margin
            ...
            unrel_span_mask = ~self.sel_stats['is_rel'][unlabel_guid,:].bool()
            real_span_mask = F.pad(ul_real_span_mask_ltoken, (0, unrel_span_mask.shape[1] - ul_real_span_mask_ltoken.shape[1]), value=0).bool()
            update_mask = unrel_span_mask & real_span_mask
            zero_tensor = torch.zeros_like(update_rate)
            update_rate = torch.where(update_mask.unsqueeze(-1), update_rate, zero_tensor)
            
            self.loss_fn_ul.confidence_update(shape=probs_neigh.shape, batch_index=unlabel_guid, update_rate=update_rate)

        output['ul_soft_probs'] = self.loss_fn_ul.confidence[unlabel_guid, :ul_all_span_logits.shape[1]][ul_real_span_mask_ltoken.bool()]
        ...
        loss_ul = self.loss_fn_ul(ul_predict_probs, unlabel_guid, selected_ul_real_span_mask_ltoken.bool(), ul_all_span_weights, ul_all_span_idxs)
    ...
    def on_train_epoch_end(self):
        ...
        self.fwrite_epoch_res.write("train%d: %f, %f, %f, %d, %d, %d\n" % (self.current_epoch, f1, recall, precision, correct_pred, total_pred, total_golden))
        
        ul_soft_probs = torch.cat([x[f'ul_soft_probs'] for x in outputs], dim=0).cuda().detach()
        self.ul_cls_n_list = ul_soft_probs.sum(dim=0)
        self.ul_cls_p_list = self.ul_cls_n_list / self.ul_cls_n_list.sum()
        self.ul_m_list = (self.args.ul_tau * torch.log(self.ul_cls_p_list))
        ...
        all_q_reps_s = torch.cat([x['src_q_reps'] for x in outputs]).cpu().detach().numpy()
        all_q_labels_s = torch.cat([x['src_q_labels'] for x in outputs]).cpu().detach().numpy()
        ...
        all_q_reps = np.append(all_q_reps_s, all_q_reps_t, axis=0)
        all_q_labels = np.append(all_q_labels_s, all_q_labels_t, axis=0)
        
        faiss_index = faiss.IndexFlatIP(self.args.q_dim)
        # faiss_index = faiss.index_cpu_to_all_gpus(faiss_index)
        gpus = [int(x) for x in str(self.args.gpus).split(",") if x.strip()]
        faiss_index = faiss.index_cpu_to_gpus_list(faiss_index, gpus=gpus)
    ...
    def validation_step(self, batch, batch_idx, mode='val'):
        ...
        all_span_logits = self.dist_model(q.view(-1, self.args.q_dim))
        all_span_logits = all_span_logits.view(span_label_ltoken.shape[0], span_label_ltoken.shape[1], -1)
        all_span_logits = all_span_logits.to(device=span_label_ltoken.device)
        ...
        if self.args.use_prune:
            span_f1s, pred_label_idx = span_f1_prune(all_span_idxs, predict_probs, span_label_ltoken, real_span_mask_ltoken)
    ...
    def on_validation_epoch_end(self):
        ...
        print("f1, recall, precision, correct_pred, total_pred, total_golden")
        print("dev%d: %f, %f, %f, %d, %d, %d\n"%(self.current_epoch, f1, recall, precision, correct_pred, total_pred, total_golden))
        self.fwrite_epoch_res.write("dev%d: %f, %f, %f, %d, %d, %d\n"%(self.current_epoch, f1, recall, precision, correct_pred, total_pred, total_golden))

        if f1 > self.best_f1 and self.args.save_best_model:
            self.best_f1 = f1
            torch.save(self.state_dict(), self.ckpt_path)
            print(f"New best model saved with F1 score: {self.best_f1:.4f}")
    ...
    def train_dataloader(self) -> DataLoader:
        ...
        self.train_class_counts = train_loader.dataset.class_counts
        return train_loader
    ...
    def get_dataloader(self, prefix="train") -> DataLoader:
        ...
        return_guid = True if prefix == 'unlabel' or prefix == 'train' else False
        if prefix == 'unlabel':
...
def main():
    ...
    devices = [int(x) for x in str(args.gpus).split(",") if x.strip()]
    ...
    elif 'multiconer' in args.dataname:
        label2idx = {"O": 0, "PER": 1, "LOC": 2, "GRP": 3, "PROD": 4, "CW": 5, "MED": 6}
    ...
    if args.ul_tau > args.tau:
        args.tau = args.ul_tau
    ...
    fp_filter_result = args.default_root_dir + '/filter_results.txt'
    args.fp_filter_result = fp_filter_result
    ...
    trainer = Trainer(accelerator='gpu', devices=devices, max_epochs=args.max_epochs, precision=args.precision, val_check_interval=args.val_check_interval, 
