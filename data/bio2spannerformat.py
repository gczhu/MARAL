...
def keep_spanPred_data(dataname, lang, fpath_bio, suffix, column_no, delimiter):
	word_seqs, trueTag_seqs, word_seqs_sent, trueTag_seqs_sent = read_data(
		dataname, lang, fpath_bio, suffix, column_no = column_no, delimiter=delimiter)  # column_no=3 for ontonotes5.0
...
def read_data(corpus_type, lang, fn, suffix, column_no=-1, delimiter=' '):
	...
	for k in range(len(lines)):
		...
		if corpus_type == 'wikiann':
			line = line.replace(f"{lang}:", "", 1)
		# if corpus_type == 'multiconer':
		# 	if '# id' in line:
		# 		continue
		# 	else:
...
if __name__ == '__main__':
	...
	if dataname =='ontonote5':
		...
		column_no = 1
		delimiter = '\t'
	elif 'wikiann' in dataname or 'conll03' in dataname or 'multiconer' in dataname:
		delimiter = '\t'
	...
	for suffix in suffixs:
		...
		if suffix == 'train':
			...
			lang1 = lang
		all_data = keep_spanPred_data(dataname, lang1, fpath_bio, suffix, column_no, delimiter)
